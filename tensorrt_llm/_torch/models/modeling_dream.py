...
class DreamGenerationMixin:
    ...
    def _validate_generated_length(self, generation_config, input_ids_length,
        ...
        logger.info(
            f"validate_generated_length: {generation_config.max_length}")
    ...
    def _prepare_generated_length(
        ...
        logger.info(
            f"prepare_generated_length: {generation_config.max_new_tokens}")
    ...
    def _prepare_generation_config(
        ...
        if not is_torchdynamo_compiling():
            ...
            if not using_model_generation_config and hasattr(
    ...
    def _prepare_special_tokens(
        ...
        def _tensor_or_none(token, device=None):
            ...
            device = device if device is not None else (getattr(
                self, 'device', None) if hasattr(self, 'device') else None)
    ...
    def diffusion_generate(
        ...
        **kwargs,
        ...
        # 4. Check input_ids
        if not is_torchdynamo_compiling() and hasattr(
                self, 'device') and self.device.type != input_ids.device.type:
            warnings.warn(
    ...
    def _sample(
        ...
        generation_config: DreamGenerationConfig, generation_tokens_hook_func,
        ...
        for i in range(steps):
            ...
            if alg == 'origin':
                ...
                device = getattr(self, 'device', x.device) if hasattr(
                    self, 'device') else x.device
                x0 = torch.zeros_like(x[mask_index],
                                      device=device,
                                      dtype=torch.long) + mask_token_id
                transfer_index_t_s = torch.rand(*x0.shape,
                                                device=device) < p_transfer
                ...
                if number_transfer_tokens > 0:
                    ...
                    x_ = torch.zeros_like(x, device=device,
                                          dtype=torch.long) + mask_token_id
                    x_[mask_index] = x0.clone()
                    row_indices = torch.arange(
                        x.size(0),
                        device=device).unsqueeze(1).expand_as(transfer_index)
...
class DreamRotaryEmbedding(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        # TODO remove the `if` below, only used for BC
        self.rope_kwargs = {}
        if config is None:
            logger.warning(
    ...
    def forward(self, x, position_ids):
        ...
        if hasattr(self.inv_freq, 'data'):
            inv_freq_expanded = self.inv_freq.data[None, :,
                                                   None].float().expand(
...
class DreamPreTrainedModel(DecoderModel):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # NOTE we need to override the generation config
...
class DreamBaseModel(DreamPreTrainedModel):
    ...
    def __init__(self, config: DreamConfig):
        ...
        self.embed_tokens = Embedding(
            ...
            if hasattr(model_config, 'mapping') else None,
            tensor_parallel_mode=TensorParallelMode.COLUMN if hasattr(
                model_config, 'mapping') else None,
            gather_output=True if hasattr(model_config, 'mapping') else None,
        )
        self.layers = nn.ModuleList([
            DreamDecoderLayer(model_config, layer_idx)
    ...
    def load_weights(self, weights: Dict):
        ...
        logger.info(
            f"Starting to load {len(weights)} weights into DreamBaseModel")
        ...
        for key, tensor in weights.items():
            if key.startswith("model.layers.") and "self_attn" in key:
                ...
                elif key.endswith("k_proj.weight") or key.endswith(
                        "v_proj.weight"):
                    # Skip these as they're handled above
                    continue
                elif key.endswith("o_proj.weight"):
                    converted_weights[key] = tensor
                elif key.endswith("q_proj.bias") or key.endswith(
                        "k_proj.bias") or key.endswith("v_proj.bias"):
                ...
                if key.endswith("gate_proj.weight"):
                    ...
                    new_key = key.replace("gate_proj.weight",
                                          "gate_up_proj.weight")
        ...
        for key, tensor in converted_weights.items():
            if hasattr(self, 'config') and hasattr(self.config, 'torch_dtype'):
                ...
                if tensor.dtype != target_dtype:
                    logger.info(
                        f"Converting {key} from {tensor.dtype} to {target_dtype}"
                    )
        ...
        logger.info(
            f"Successfully loaded {len(converted_weights)} weights into DreamBaseModel"
        )
        ...
        **kwargs,
        ...
        if hasattr(self, 'config') and hasattr(self.config, 'torch_dtype'):
            ...
            if hidden_states.dtype != target_dtype:
                logger.info(
                    f"Converting hidden_states from {hidden_states.dtype} to {target_dtype}"
                )
...
class DreamForCausalLM(DecoderModelForCausalLM[DreamModel, DreamConfig]):
    ...
    def __init__(self, model_config: ModelConfig[DreamConfig]):
    ...
    def load_weights(self, weights: Dict, skip_modules: List[str] = []):
        ...
        logger.info(
            f"Starting to load {len(weights)} weights into DreamForCausalLM")
        ...
        logger.info(
            f"Successfully loaded {len(converted_weights)} weights into DreamForCausalLM"
        )
        ...
        **kwargs,
        ...
        if not return_context_logits:
            if attn_metadata is not None and attn_metadata.seq_lens_cuda is not None:
                ...
                is_block_prediction = any(
                    seq_len > 1 for seq_len in attn_metadata.seq_lens_cuda)
        ...
        if isinstance(return_context_logits,
                      int) and not isinstance(return_context_logits, bool):
            last_tokens = torch.cumsum(
                attn_metadata.seq_lens_cuda,
                dim=0,
                dtype=torch.long,
            ...
            offsets = torch.arange(-return_context_logits,
                                   0,
            ...
            hidden_states = torch.gather(
                hidden_states, 0,
                gather_indices.flatten()[:, None].repeat(1, d_model))
